...
func (l *Life) Render(s *user.Session) {
	cell := screen.Character{81, screen.White, screen.DarkGray, false}
	empty := screen.Character{32, screen.White, screen.DarkGray, false}

	for row := 0; row < l.h; row++ {
		for col := 0; col < l.w; col++ {
			if l.a.Alive(col, row) {
				s.Buffer.Write(cell, uint8(row)+1, uint8(col)+1)
			} else {
				s.Buffer.Write(empty, uint8(row)+1, uint8(col)+1)
			}
		}
	}
}
