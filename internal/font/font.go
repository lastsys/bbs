...
// Save the image to static web resources.
func saveImage(img *image.RGBA) {
	f, err := os.Create("./web/static/font.png")
	if err != nil {
		panic(err)
	}
	defer f.Close()
